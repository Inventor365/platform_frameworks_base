...
constexpr std::string_view kLuminePath = "/system/framework/org.luminedroid.platform-res.apk";
...
idmap2::Result<Idmap2Service::TargetResourceContainerPtr> Idmap2Service::GetTargetContainer(
  ...
  const bool is_lumine_framework = target_path == kLuminePath;
  bool use_cache;
  struct stat st = {};
  if (is_framework || is_lumine_framework || !::stat(target_path.c_str(), &st)) {
    use_cache = true;
  } else {
